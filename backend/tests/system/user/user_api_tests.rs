...
async fn create_test_app() -> (axum::Router, TestDb) {
    ...
    let sys_dict_item_service =
        Arc::new(SysDictItemService::new(sys_dict_item_repo, sys_dict_repo.clone()));
...
async fn test_create_user_sync_harbor_failure_rolls_back_local_user() {
    ...
    assert!(
        ...
        "Expected Harbor sync failure to return an error"
    );
    ...
    let body: Bytes = to_bytes(get_response.into_body(), 1024 * 1024)
        .await
        .unwrap();
    let json: Value = serde_json::from_slice(&body).unwrap();
    let data = json.get("data").unwrap();
    ...
    assert_eq!(
        total, 0,
        "Local user should not be created when Harbor sync fails"
    );
}
